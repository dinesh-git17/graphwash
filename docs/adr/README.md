...
Michael Nygard minimal format. Each ADR captures one irreversible (or costly-to-reverse) decision.
...
- A decision that replaces an existing ADR supersedes it — the old ADR's status becomes `Superseded by NNNN` and the new ADR notes what it supersedes.
