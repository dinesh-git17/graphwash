...
- **Unknown (CLOSED 2026-04-21):** Does the auth-adjacent fallback (per-IP rate limit at the proxy) work with a short Caddy config, and does UptimeRobot's free tier actually support the 2-min probe cadence assumed by §17 monitoring? Closed by T-020; rate-limit confirmed per-IP, and UptimeRobot free tier floors at 5 min, not 2 min.
- **Method (as executed):** (a) Rebuilt Caddy with `mholt/caddy-ratelimit` via `xcaddy 0.4.5` and added a per-IP `rate_limit` block (10 events / 1 s, key `{remote_host}`) to `ops/graphwash.Caddyfile`. Concurrent burst test from a laptop returned 180 of 200 requests as 429, while a concurrent VPS-origin curl loop received 20 of 20 as 200, confirming per-IP scope. (b) Created a UptimeRobot keyword monitor on `/api/v1/health` (alert when `ok` absent, 5-min interval); kill-signal verified with deliberate `docker stop graphwash`; DOWN email arrived 2 m 57 s after stop, well inside the 5-min budget; UP email arrived 3 m 35 s after restart.
...
- **Outcome:** All gates cleared. UptimeRobot free-tier 2-min cadence assumption was wrong (floor is 5 min); accepted the 5-min cadence and amended §17 monitoring threshold from `> 2 minutes` to `> 5 minutes`. §18 gains an open question on vendor choice for v1.0.
- **Owner:** Dinesh.
- **Evidence:** `docs/ops/monitoring.md`, `ops/deploy-caddy.md`, `ops/graphwash.Caddyfile`.
...
| -------------------------------- | --------------------------- | --------------------- |
| `GET /api/v1/health` HTTP status | ≠ 200 for > 5 minutes       | Rollback (P0 alert)   |
...
Tooling (v1): UptimeRobot monitors `/api/v1/health`; `docker logs` tailed via SSH; a weekly manual F1 eval script re-runs evaluation against the held-out test split. Alerting: email to `dind.dev@gmail.com` on any UptimeRobot P0. The 5-min health-check threshold matches UptimeRobot free-tier minimum probe interval (T-020 closure 2026-04-21).
...
| Stay on UptimeRobot free tier for v1.0, or migrate to Grafana Cloud free tier (or another vendor) for richer dashboards and alerting policy? | T-020 spike validated UptimeRobot for the Pilot. The 5-min cadence and single-region probe are acceptable for a no-SLA portfolio demo, but v1.0 may want tighter cadence, multi-region probes, or paid SSL-expiry monitoring. | Dinesh     | Before Phase 5b ship gate (T-067 area).                                                                   |
